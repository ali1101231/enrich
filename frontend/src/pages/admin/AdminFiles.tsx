...
import { Search, Download, FileText, Trash2, User } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Card, CardContent } from '@/components/ui/card';
import { Input } from '@/components/ui/input';
import { Checkbox } from '@/components/ui/checkbox';
import { Badge } from '@/components/ui/badge';
import { useAdminAllExports, useAdminDeleteExport, useAdminBulkDeleteExports } from '@/hooks/useApi';
...
export default function AdminFiles() {
  ...
  const { data: exports = [] } = useAdminAllExports();
  const deleteExportMut = useAdminDeleteExport();
  const bulkDeleteMut = useAdminBulkDeleteExports();
  ...
  const filtered = exports.filter(exp => {
    ...
    return (
      ...
      (exp.user.displayName ?? '').toLowerCase().includes(query)
    );
  });
    ...
    <div className="p-6 lg:p-8 space-y-6 animate-fade-in">
      <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={0} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">View and manage exported files from all users</p>
        </div>
        {selected.size > 0 && (
          <Button
            variant="destructive"
            ...
            {filtered.length === 0 ? (
              ...
              <div className="divide-y">
                ...
                {filtered.map((exp, i) => {
                  ...
                  const userName = exp.user.displayName || exp.user.email;
                  return (
                    <motion.div
                      key={exp.id}
                      ...
                      onClick={() => navigate(`/admin/activity/${exp.batchId}`)}
                    >
                      <Checkbox
                        checked={isSelected}

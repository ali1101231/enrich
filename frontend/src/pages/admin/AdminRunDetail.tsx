...
import {
  ...
  useAdminBatchJobs,
  useAdminBatchExports,
  useAdminExportCsv,
  useAdminBatchResultCounts,
  useAdminAllBatches,
} from '@/hooks/useApi';
...
export default function AdminRunDetail() {
  ...
  const { data: batch, isLoading: batchLoading } = useAdminBatchDetail(batchId);
  const { data: progress } = useAdminBatchProgress(batchId);
  const { data: jobsData } = useAdminBatchJobs(batchId);
  const { data: exports = [] } = useAdminBatchExports(batchId);
  const { data: resultCounts } = useAdminBatchResultCounts(batchId);
  const exportMutation = useAdminExportCsv();

  // Find user info from the all-batches cache
  const { data: allBatches = [] } = useAdminAllBatches();
  const batchMeta = allBatches.find(b => b.id === batchId);
  ...
  if (!batch) {
    return (
      <div className="p-8 text-center">
        ...
        <Button variant="link" onClick={() => navigate('/admin/activity')}>
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1 text-muted-foreground text-sm">
            ...
            <span>{batchMeta?.userDisplayName ?? batchMeta?.userEmail ?? 'Unknown user'}</span>
            {batchMeta?.userEmail && batchMeta.userDisplayName && (
              <span className="text-xs">({batchMeta.userEmail})</span>
            )}
          </div>
        </div>
      </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <p className="font-medium">{batchMeta?.userDisplayName ?? batchMeta?.userEmail ?? '—'}</p>
                {batchMeta?.userEmail && batchMeta.userDisplayName && (
                  <p className="text-xs text-muted-foreground">{batchMeta.userEmail}</p>
                )}
              </div>
              <div>
              ...
              {batchMeta?.toolId && (
                <div>
                  ...
                  <p className="font-medium">{batchMeta.toolId}</p>
                </div>
              )}
            </CardContent>
          </Card>
